...
export default function BrandComparisonChart({ filters }: { filters: FilterState }) {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const res = await dashboardApi.brandComparison(params)
    ...
    const loadBrandTopics = async () => {
        ...
        try {
            ...
            const res = await dashboardApi.brandTopics(params)
    ...
    // Reset selected brand when filters change
    useEffect(() => {
        setSelectedBrand('')
    }, [filters.regionId, filters.countryId, filters.ibVersionId, filters.brandModel, filters.surveyLocation, filters.dateFrom, filters.dateTo, filters.search])
    // Get the selected brand's topic data
    const selectedBrandData = brandTopics.find((b) => b.brand === selectedBrand)

    // Convert passive topics object to array for chart
    ...
    // Convert issues topics object to array for chart
    ...
    const SingleTooltip = ({ active, payload, color, metricName }: any) => {
        if (active && payload && payload.length) {
            const item = payload[0].payload as BrandComparisonData
            return (
                <Box sx={{
                    ...
                    <Typography variant="body2" sx={{ color, fontWeight: 600 }}>
                        {metricName}: {item[metricName === 'Passive' ? 'passive_count' : 'issues_count'].toLocaleString()}
                    </Typography>
                </Box>
            )
        }
    ...
    // Custom tooltip for topic charts
    ...
    return (
        <Box>
            ...
            {/* Two Separate Charts: Passive & Issues (per brand totals) */}
            <Grid container spacing={2} sx={{ mb: 2 }}>
                {/* Passive Chart */}
                <Grid size={{ xs: 12, md: 6 }}>
                    <Card>
                        <CardContent>
                            <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: PASSIVE_COLOR }}>
                                Passive (Good) by Brand
                            </Typography>
                            {loading ? (
                                <Box sx={{ display: 'flex', justifyContent: 'center', py: 6 }}>
                                    <CircularProgress sx={{ color: c.primary }} />
                                ...
                                <ResponsiveContainer width="100%" height={chartHeight}>
                                    ...
                                    >
                                        ...
                                        <YAxis
                                            ...
                                            width={100}
                                        />
                                        <Tooltip content={<SingleTooltip color={PASSIVE_COLOR} metricName="Passive" />} />
                                        <Bar dataKey="passive_count" fill={PASSIVE_COLOR} radius={[0, 4, 4, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            )}
                        </CardContent>
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                    <Card>
                        <CardContent>
                            <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: ISSUES_COLOR }}>
                                Issues (Complaints) by Brand
                            </Typography>
                            {loading ? (
                                <Box sx={{ display: 'flex', justifyContent: 'center', py: 6 }}>
                                    <CircularProgress sx={{ color: c.primary }} />
                                ...
                                <ResponsiveContainer width="100%" height={chartHeight}>
                                    ...
                                    >
                                        ...
                                        <YAxis
                                            ...
                                            width={100}
                                        />
                                        <Tooltip content={<SingleTooltip color={ISSUES_COLOR} metricName="Issues" />} />
                                        <Bar dataKey="issues_count" fill={ISSUES_COLOR} radius={[0, 4, 4, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            )}
                        </CardContent>
